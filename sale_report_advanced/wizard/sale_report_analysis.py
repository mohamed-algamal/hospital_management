...
class SaleReportAnalysis(models.TransientModel):
    ...
    def action_get_excel_analysis_report(self):
        ...
        datas = self._get_data()
        return self.env.ref('sale_report_advanced.action_sale_analysis_report_xlsx').report_action([], data=datas)

class SaleAnalysisReportXlsx(models.AbstractModel):
    _name = 'report.sale_report_advanced.report_sale_analysis_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        loaded_data = data
        sheet = workbook.add_worksheet("Sales Analysis Report")
        cell_format = workbook.add_format(
            {'font_size': 12, 'bold': True, 'align': 'left', 'valign': 'vcenter', 'font_color': '#333333'})
        head = workbook.add_format(
            {'font_size': 20, 'bold': True, 'align': 'center', 'valign': 'vcenter', 'font_color': '#1F4E78',
             'border': 0})
        txt = workbook.add_format({'font_size': 10, 'align': 'center', 'valign': 'vcenter', 'font_color': '#000000'})
