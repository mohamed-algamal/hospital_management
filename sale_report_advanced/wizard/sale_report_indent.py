...
class SaleReportIndent(models.TransientModel):
    ...
    def action_get_excel_indent_report(self):
        """ Generate an Excel category-based sales report.
            ...
            """
        datas = self._get_data()
        return self.env.ref('sale_report_advanced.action_sale_indent_report_xlsx').report_action([], data=datas)

class SaleIndentReportXlsx(models.AbstractModel):
    _name = 'report.sale_report_advanced.report_sale_indent_xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        """ Generate an Excel category-based sales report. """
        loaded_data = data
        sheet = workbook.add_worksheet("Sales Indent Report")
        ...
        format1 = workbook.add_format({'font_size': 10, 'align': 'center', 'bg_color': '#f5f9ff', 'border': 1})
        ...
        for rec in partner:
            count = 0
            h_row += 1
            sheet.merge_range(h_row, h_col - 5, h_row, h_col, rec['name'], format3)
            c_row = c_row + 2
            row += 2
            row_number += 2
            for cat in categ:
                ...
                c_col = 12
                col = 9
                sheet.merge_range(c_row, c_col-5, c_row, c_col, cat['name'], format1)
                sheet.merge_range(row, c_col-5,row, c_col-3, 'Product', format4)
                sheet.set_column('J:J', 17)
                col += 1
                sheet.merge_range(row, c_col-2, row, c_col, 'Quantity', format4)
